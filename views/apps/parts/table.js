...
for (let x = 0; x < workTime2inmin.length; x++) {
    ...
    if(workTime2inmin[x]%meetingT == 0){
        ...
        for(let i = 0;i <  (workTime2inmin[x]/meetingT); i++){
            ...
            inputsDev2.innerHTML += `<div class="parttimes" id=""> ${unikTime.getHours()}:${unikTime.getMinutes()} - ${uniktime2.getHours()}:${uniktime2.getMinutes()}</div>`
...
function transferValue(){
    ...
    var timeAPInput = document.getElementById("timeAP"); // input that get time from table
    ...
    function singleSelect(li){
        ...
        timeAPInput.value = li.innerHTML;
        showTime.style.opacity = "1";
        showTime.innerHTML = `Your chosen time is:<br> <span id="hourmin"> ${li.innerHTML}</span>`;
...
function dateChanges(){
    ...
    for (let i = 0; i <= 6; i++) {
        ...
        weekDays.innerHTML += `<div class="weekdays">${daysName[i]} / ${newdate.getDate()} / ${monthNames[newdate.getMonth()]}</div>`;
    }
